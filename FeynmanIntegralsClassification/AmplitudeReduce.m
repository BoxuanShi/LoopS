...
AmplitudeReduce[amp_, loops_List, familyLS_List, indices_List, operatorRules : _List|_Dispatch, loopmoms_List, moms_List, extmomsind_List, purePV_String, kinematics_List, opt : OptionsPattern[]] := Module[{i, j, k, tpNumlist, tpDenolist, tpNumSPD, PowerCounting0, PowerCounting, tp1, tp2, optnum, optFR, OPERAT, ophead, opttable},
  (*simple cases*)
  If[amp === 0,
    Switch[
      OptionValue["AmplitudeReduceForm"],
  ...
  Switch[OptionValue["AmplitudeReduceForm"], 
    ...
    If[Length@tp2 === 1,
      ...
      Table[tp2[[i, {1, 3}]] = tp2[[i, {1, 3}]] /. Dispatch@tpNumSPD[[i, 2]], {i, Length@tp2}];
      tp2 = AbbreviatePolynomials[tp2, OperatorPattern, _OPERAT];
      {tp2[[1]], Dispatch[tp2[[2]] /. Dispatch[OPERAT -> ophead]]}
      ],

    "List",
    Table[tp2[[i, {1, 3}]] = tp2[[i, {1, 3}]] /. Dispatch@tpNumSPD[[i, 2]] /. Dispatch[OPERAT -> ophead], {i, Length@tp2}];
    tp2,
    ...
    If[Length@tp2 === 1,
      ...
      Table[tp2[[i, {1, 3}]] = tp2[[i, {1, 3}]] /. Dispatch@tpNumSPD[[i, 2]], {i, Length@tp2}];
      tp2 = AbbreviatePolynomials[tp2, OperatorPattern, _OPERAT];
      {Total[Dot @@@ tp2[[1]]] // OptionValue["AmplitudeReduceSimplify"], Dispatch[tp2[[2]] /. Dispatch[OPERAT -> ophead]]}
      ]
