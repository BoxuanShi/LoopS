...
zeroSectorQ[pdlist0_, loops_List, process_Association] := 
 zeroSectorQ[pdlist0, loops, process["kinematics"]]
zeroSectorQ[pdlist0_List, loops_List, SPRep_List] := 
 Module[{pdlist, i, pos, n = Length@pdlist0, flag, G, x, k, xprod, eq, eqlist,
    terms, mat, redmat},
      (*pdlist=Cases[pdlist0,a_/;FreeQ[Denominator[a],Alternatives@@loops]];*)
      pdlist = 
   Union@Cases[pdlist0, a_ /; FreeQ[Denominator[a], Alternatives @@ loops]];
  	...
  	(*Eq.(16) of 1310.1145: Criterion is Sum[k[i]x[i]D[G,x[i]],{i,n}] = 
  G with G=F+U*)	
  	...
  	mat = Append[Coefficient[eqlist, #] & /@ Array[k, n], eqlist /. _k -> 0] //
     Transpose;
  	(*solve equations*)
...
zeroSectorQ[pdlist_FAD | pdlist_SFAD, loops_, kinematics_] := 
 zeroSectorQ[FADToProps[pdlist, List], loops, kinematics]