...
MIsollistNLO = {
   ...
   Module[{s1 = a1 a2}, 
    1/\[Epsilon] + (2 - Log[-s1]) + 
     1/12 \[Epsilon] (6 Log[-s1]^2 - 24 Log[-s1] - \[Pi]^2 + 
        48) + \[Epsilon]order[2, 2]*\[Epsilon]^2],
   Module[{s1 = (a1 a2)/(a1 a2 - a3), s2 = a1 a2 - a3},
    (*1/\[Epsilon]+(2+(-1+1/s1) Log[1-s1])+1/(
    12 s1)(s1 (48+\[Pi]^2)-24 (-1+s1) Log[1-s1]+6 (-1+s1) Log[1-
    s1]^2-12 (-1+s1) PolyLog[2,s1/(-1+
    s1)]) \[Epsilon]+\[Epsilon]order[3,2]*\[Epsilon]^2;*)
    1/\[Epsilon] + (2 - 
       Log[a1 a2 - a3] + (-1 + 1/s1) Log[1 - s1]) + (1/
         2 Log[a1 a2 - a3]^2 - 
        Log[a1 a2 - 
   ...
   Module[{s3, x, y},
    s3 = (a1 - b1) (a2 - b2); {x, y} = {a1/(a1 - b1), a2/(
      a2 - b2)}; -((
      PolyLog[2, x/y] + PolyLog[2, (1 - y)/(1 - x)] + 
       PolyLog[2, ((1 - x) y)/(x (1 - y))] - PolyLog[2, y/x] - 
       PolyLog[2, (1 - x)/(1 - y)] - 
       PolyLog[2, (
        x (1 - y))/((1 - x) y)])/((x - y) (-s3))) + \[Epsilon]order[4,
        1]*\[Epsilon]]};