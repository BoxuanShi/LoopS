...
Protect[AbbrP];
Options[AbbreviatePolynomials] := CreateOptions[{"AbbreviatePolynomialsName" -> AbbrP, "AbbreviatePolynomialsHead" -> (# &)}, {}];

AbbreviatePolynomials[expr0_List, patt_, opt : OptionsPattern[]] := 
 Module[{expr, expr2, Op, rules, rulesInv, OpName, hd},
  ...
  rulesInv = Thread[OpName -> (Op /. hd -> OptionValue["AbbreviatePolynomialsHead"])];
  {expr2, rulesInv}]

AbbreviatePolynomials[expr_, patt_, opt : OptionsPattern[]] := Module[{Coe, Op, rules, OpName, nQ},
  {Coe, Op} = Separate[expr, patt];
  nQ = (Coe =!= {} && Op[[1]] === 1);
  Op = (OptionValue["AbbreviatePolynomialsHead"] /@ Op) /. OptionValue["AbbreviatePolynomialsHead"][1] -> 1;
