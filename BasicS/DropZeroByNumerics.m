...
DropZeroByNumerics[expr0_, expr_, patt_] := 
 Module[{tp1, poly1, vars, func1, poly2, poly3, poly4, tpRules2},
  poly1 = expr // getS[#, patt] &;
  tp1 = expr // SeparatePoly[#, poly1, "SeparateDropOne" -> True] &;
  If[poly1 =!= {} && poly1 =!= tp1[[2]], 
...
patt."]; Abort[]];
  ...
  poly4 = Transpose@tp1 // Select[#, MemberQ[poly3, #[[2]]] &] &;
  poly4 = poly4 // TogetherExpand;
  poly4 = Select[poly4, #[[1]] === 0 &][[All, 2]];
  tpRules2 = Dispatch@Thread[(Verbatim /@ poly4) -> 0];
  expr0 /. tpRules2]