...
GDerivative[expr_, var_, family_, process_String : "CurrentProcess"] := GDerivative[expr, var, family, ToExpression[process]]

GDerivative[expr_, var_, family_, process_Association] := GDerivative[expr, var, family, process["loopmoms"], process["kinematics"], process["moms"]]
...
GDerivativeG[expr_G, var_, family_, loopmoms_, kinematics_, moms_] := 
 Module[{familyN, exprSPD, exprD, exprA, rules, coe, deno, Abbr}, 
  {familyN, exprSPD} = {expr[[1]], GToProps[expr, family] // PropsToSPD // Times @@ #^-1 &};
  exprD = D[exprSPD, var];
