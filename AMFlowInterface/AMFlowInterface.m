...
output=Table[FileNameJoin[{WorkPath,SaveName<>ToString[i]<>".wl"}],{i,Length@family}];
save=Table[FileNameJoin[{WorkPath,SaveName<>ToString[i]}],{i,Length@family}];
...
If[!SubsetQ[Numeric[[All,1]],Complement[Variables[family],Flatten[{loops,Variables@Normal[kinematics][[All,1]]}]]],Print[Numeric0," is not complete."];Abort[]];
...
Print[(SaveName<>ToString[i])," is already defined. Please Clear it firstly."];Abort[]];

FileTemplateApply[AMFTemplate,

<|"AMFlow"->ToStringInput@amfinstall,"IBPReducer"->ToStringInput["FIRE+LiteRed"],"Family"->(SaveName<>ToString[i]),"loops"->ToStringInput@loops,"extmomsind"->ToStringInput@extmomsind,"kinematics"->ToStringInput@kinematics,"family"->ToStringInput@(family[[i]]/. Rationalize@Numeric),"Numeric"->ToStringInput@Numeric,"NThread"->ToStringInput@NThread,"target"->ToStringInput@target[[i]],"goal"->ToStringInput@goal,"epsorder"->ToStringInput@epsorder,"SolveIntegralsGaugeLink"->solve[[i]],"save"->ToStringInput[save[[i]]]|>,
...
ClearAll[GToj,jToG];
GToj[expr_,SaveName_String]:=expr/.G[a_,b_]:>j[SaveName<>ToString[a]//ToExpression,Sequence@@b]
...
AMFlowSaveName[loops_List]:=Module[{str,i},str=StringJoin@@Table["N",{i,Length[loops]}];
"AMFfamily"<>str<>"LO"]