...
class DB {
 public:
   ...
   */
  virtual int64_t get(const char* kbuf, size_t ksiz, char* vbuf, size_t max) = 0;
  /**
   ...
   * @return the size of the value, or -1 on failure.
   */
  virtual int64_t check(const char* kbuf, size_t ksiz) = 0;
  /**
   * Check the existence of a record.
   * @note Equal to the original DB::check method except that the parameter is std::string.
   */
  virtual int64_t check(const std::string& key) = 0;
  /**
   * Remove all records.
   * @return true on success, or false on failure.
   */
...
class BasicDB : public DB {
 ...
 public:
  ...
  enum OpenMode {
    ...
    OREORGANIZE = 1 << 9                  //force file reorganize
  };
  /**
   * Destructor.
   ...
   * @return the size of the value, or -1 on failure.
   */
  int64_t get(const char* kbuf, size_t ksiz, char* vbuf, size_t max) {
   ...
   * @return the size of the value, or -1 on failure.
   */
  int64_t check(const char* kbuf, size_t ksiz) {
    class VisitorImpl : public Visitor {
     public:
   ...
   * @note Equal to the original DB::check method except that the parameter is std::string.
   */
  int64_t check(const std::string& key) {
