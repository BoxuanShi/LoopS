...
class CacheDB : public BasicDB {
 ...
 public:
  ...
  bool accept(const char* kbuf, size_t ksiz, Visitor* visitor, bool writable = true) {
    ...
    if (parallel_access) ScopedRWLock lock(&mlock_, false);
    if (omode_ == 0) {
    ...
    if (parallel_access) slot->lock.lock();
    accept_impl(slot, hash, kbuf, ksiz, visitor, comp_, rttmode_);
    if (parallel_access) slot->lock.unlock();
    return true;
  }
  /**
 ...
 private:
  ...
  uint64_t hash_record(const char* kbuf, size_t ksiz) {
    _assert_(kbuf && ksiz <= MEMMAXSIZ);
    if (ksiz == 16) return (*reinterpret_cast<const uint64_t*>(kbuf) ^ *reinterpret_cast<const uint64_t*>(kbuf+8));
