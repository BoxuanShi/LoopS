...
namespace CoCoA
{
  ...
  matrix LinKerZZ(ConstMatrixView M)
  {
    ...
    for (int j=0; j < ncols; ++j)
    {
      ...
      const BigInt D = CommonDenom(col_j);
      for (int i=0; i < nrows; ++i)
        M_ZZ[j][i] = num(col_j[i])*(D/den(col_j[i]));
    }
    const auto K = ZZKernel(M_ZZ);
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/MatrixOps-LinKerZZ.C,v 1.3 2021/04/26 13:57:14 abbott Exp $
