  ...
  namespace // anonymous
  {
                                ...
                                const ideal& I, VerificationLevel VerLev)
    {
      ...
      for (long i=0; i < level(VerLev); ++i)
      {
        ...
        vector<RingElem> Gp;
        if (HasGBasis(I)) Gp = apply(pi, GBasis(I));
        else Gp = GBasis(ideal(apply(pi, gens(I))));
        RingElem gp = pi(g);
        //      VERBOSE(50) << "horner-"<<i << std::endl;
        eval_f = zero(Pp);
        for (long d=len(c)-1; d>=0; --d)
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/SparsePolyOps-MinPoly.C,v 1.18 2021/02/10 19:41:44 abbott Exp $
