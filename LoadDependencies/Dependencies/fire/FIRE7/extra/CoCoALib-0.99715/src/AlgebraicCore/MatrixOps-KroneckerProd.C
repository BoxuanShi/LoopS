  ...
  matrix KroneckerProd(ConstMatrixView M1, ConstMatrixView M2)
  {
    ...
    if (R1 != R2)
    {
      ...
      if (codomain(promote) == R1)
        return KroneckerProd(M1, apply(promote,M2));
      return KroneckerProd(apply(promote,M1), M2);
    }
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/MatrixOps-KroneckerProd.C,v 1.3 2020/06/22 15:43:37 abbott Exp $
