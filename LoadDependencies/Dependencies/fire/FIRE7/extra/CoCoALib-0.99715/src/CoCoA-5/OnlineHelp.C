...
namespace OnlineHelp
{
  ...
  namespace // anonymous
  {
    ...
    std::ifstream OpenXMLManual()
    {
      ...
      if (!in)
        CoCoA_THROW_ERROR("Cannot find/read XML file "+CoCoAManFileName(), "OnlineHelp::OpenXMLManual()");
      return in;
    }
...
//     string ManDate;
  ...
  void PrintExampleWithoutOutput(std::ostream &out, std::ifstream& in)
  {
    ...
    out << "R := \"R not defined\";  ";
    out << "P := \"P not defined\";  -- clear variables" << endl;
    string line;
    getline(in, line);
    while ( !IsSubstring("</example>", line) )
    {
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/CoCoA-5/OnlineHelp.C,v 1.94 2020/10/23 08:28:11 bigatti Exp $
// $Log: OnlineHelp.C,v $
