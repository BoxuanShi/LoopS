...
countProps[props_List, loops_List] := 
 Module[{x, props1, props2, props3, inv, i, ct, tp1, tp2, tp3, tp4, 
   largePowPos, largePowProps},
  props1 = Union@props;
  inv = Table[
    If[FreeQ[Denominator@props1[[i]], Alternatives @@ loops], 1, -1],
    {i, Length@props1}];
  ...
  If[Total@ct =!= Length@props, Print["countProps: count wrong."]; 
   Abort[]];
  {props2, ct*inv}
  ]
...
EiknolPermutation[family_List, loops_List] := Module[{posL, permuL},
  ...
  If[Length@posL > 0,
   permuL = 
    Tuples[ConstantArray[{1}, Length@family] // 
      ReplacePart[#, posL -> {1, -1}] &],
...
matchFI::version = 
  "Mathematica version is less than 12.1, matchFI may gives divergent \
result.";
Options[matchFI] := 
  CreateOptions[{"ExactMode" -> False, "Head" -> MF, 
    Assumptions -> $Assumptions, "FailedReturn" -> Automatic, 
    "matchFILog" -> False}, {Solve}];
matchFI[target_List, formlist_List, loops_List, 
   process_String : "CurrentProcess", opt : OptionsPattern[]] /; 
  OptRestrict[opt] := 
 matchFI[target, formlist, loops, ToExpression[process], opt]
matchFI[target_List, formlist_List, loops_List, process_Association, 
   opt : OptionsPattern[]] /; OptRestrict[opt] := 
 matchFI[target, formlist, loops, process["kinematics"], opt]
matchFI[target0_?(Head@# =!= List &), ___] := target0
matchFI[target0_List, formlist_List, loops_List, kinematics_List, 
   opt : OptionsPattern[]] /; OptRestrict[opt] := 
 Module[{ctt, ctf, v, head, target, target1, formi, posL, propsL, 
   failedreturn, permu, permuL, num1, num2, num3, x, tp1, tp2, tp3, 
   ord1, ord2, eqs, tpInd, tpAsm, tpvars, varRules, sol, optSolve, 
   retu, i},
  
  (*If[$VersionNumber<12.1,Message[matchFI::version]];*)
  ...
  For[num3 = 1, num3 <= Length@permuL, num3++,
   target1 = target*permuL[[num3]];
   
   
   For[
    num1 = 1, num1 <= Length@formlist, num1++,
    {formi, ctf} = countProps[formlist[[num1, 1]], loops];
    If[Sort@ctt =!= Sort@ctf || 
      Length@loops =!= Length@formlist[[num1, 2]], Continue[]];
    
    tp1 = 
     SymanzikPolynomials[x, formi, 
      Sequence @@ formlist[[num1, {2, 4}]]];
    ...
    tpAsm = 
     tpAsm && 
      If[Length@formlist[[num1]] === 5, formlist[[num1, 5]], True];
    ...
    {tp1, tpAsm} = {tp1, tpAsm} /. 
      Table[x[ord1[[1, i]]] -> x[i], {i, Length@formi}];
    ...
    For[
     ...
     tp2 = 
      tp3 /. Table[x[ord2[[num2, i]]] -> x[i], {i, Length@target}];
     eqs = Thread[Separate[Total[tp2 - tp1], _x][[1]] == 0];
     If[Union[eqs /. (Reverse /@ varRules)] === {True}, 
      Return[head[num1, {}]]];(*exact return*)
     
     If[! OptionValue["ExactMode"],
      If[
       OptionValue[Assumptions] === True && 
        Length@formlist[[num1]] === 4,
       optSolve = Sequence[],
       optSolve = FilterOptions[{opt}, Solve]];
      
      If[$VersionNumber < 12.1,
       sol = Solve[eqs, Array[v, Length@tpvars], optSolve](*//
       TimingS*);
       Message[matchFI::version]
       ,
       sol = 
        Solve[eqs, Array[v, Length@tpvars], Assumptions -> tpAsm, 
         optSolve](*//TimingS*)
       ],
      Continue[]
     ...
     If[
      ...
      (*when there are variable with the same name in target and \
formi, they will be regard as the same, 
      the conditional expression may be undefined like: x>0&&x<0*)
      sol = sol[[1]] /. (Reverse /@ varRules);
      sol = 
       sol // Simplify[#, 
           Assumptions -> (tpAsm /. (Reverse /@ varRules))] & // 
        DeleteCases[#, x -> x] &(*//TimingS*);
      
      (*make sure no Undefined in retu and no duplicated indepedent \
variable after transform, 
      ...
      retu = 
       Select[{retu}, 
        FreeQ[#[[2]], Undefined] && 
          DuplicateFreeQ[(FactorTermsList[#][[2]] &) /@ (tpInd /. 
              Normal[getS[#, _head][[1]][[2]]])] &];
  ...
  Return[
   If[failedreturn === Automatic, MissMatch @@ target0, 
    failedreturn]]
  ]