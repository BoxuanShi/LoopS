...
IncrementListElement[vec_List, patt_, b_Integer] := Module[{i, pos, tp1},
  ...
  tp1 = Table[vec // ReplacePart[#, pos[[i]] :> (#[[Sequence @@ pos[[i]]]] + b)] &, {i, Length@pos}];
...
MixedIndex;
PossibleMIsInSector::usage = "PossibleMIsInSector[g0_G, rx_Integer : 0, sx_List : {0, 2}, dx_List : {0, 1}, opt : OptionsPattern[]].
rx: Total of positive index (0 means no constraint), 
sx: minimum and maximum index, 
dx: minimum and maximum sum of total negative and enhanced index.
\"MixedIndex\" : True|False : False -> whether negative and enhanced indices appear at the same time.";
PossibleMIsInSector[g0_G, rx_Integer : 0, sx_List : {0, 2}, dx_List : {0, 1}, opt : OptionsPattern[]] := 
 Module[{x, num, rx2, sec, s1, s2, f1, f2, d1, d2, tp1, tp2, tp3, tp4},
  ...
  f1[x_] := Flatten[(IncrementListElement[#, Alternatives @@ Range[s1 + 1, 0], -1] &) /@ x, 1] // DeleteDuplicates;
  f2[x_] := Flatten[(IncrementListElement[#, Alternatives @@ Range[1, s2 - 1], 1] &) /@ x, 1] // DeleteDuplicates;
...
Options[PossibleIntForDEInSector] := CreateOptions[{}, {PossibleMIsInSector}];
PossibleIntForDEInSector[g0_G, rx_Integer : 0, sx_List : {0, 2}, dx_List : {0, 1}, opt : OptionsPattern[]] := Module[{vec, tp1, optPMS},
